...
@pytest.fixture
def config_file() -> Tuple[str, List[str]]:
    ...
    config_ = {
        ...
        "f4": {"s1": ["a", "", 42], "s2": ["d", "f", "ENV__VARIABLE_2"]},
        "f5": [["kek", "ENV__VARIABLE_3"], {"t1": ["lol", "ENV__VARIABLE_4"]}],
    }
    expected_vars = ["VARIABLE_0", "VARIABLE", "VARIABLE_2", "VARIABLE_3", "VARIABLE_4"]

    with open(file_name, "w") as f:
        yaml.dump(config_, f)

    yield file_name, expected_vars

    os.remove(file_name)


@pytest.fixture
def config_file_with_env(config_file: Tuple[str, List[str]]) -> Tuple[str, str]:
...
class TestConfig:
    ...
    def test_fill_from_env(self, config_file_with_env: Tuple[str, dict]):
...
def test_config_path():
    config = {"a": 1, "b": 2, "c": {"a1": 1, "b1": [1, 2, 3, {"a11": "kek"}]}}
